...
private let log = Logger(subsystem: "com.bocoup.ATATExtension", category: "AudioUnitViewController")

public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
  var audioUnit: AUAudioUnit?

  var hostingController: HostingController<ATATExtensionMainView>?
  ...
  public func createAudioUnit(with componentDescription: AudioComponentDescription) throws
    -> AUAudioUnit
  {
    audioUnit = try ATATExtensionAudioUnit(componentDescription: componentDescription, options: [])

    guard let audioUnit = self.audioUnit as? ATATExtensionAudioUnit else {
      log.error("Unable to create ATATExtensionAudioUnit")
      return audioUnit!
    }
    ...
    audioUnit.setupParameterTree(ATATExtensionParameterSpecs.createAUParameterTree())
  ...
  private func configureSwiftUIView(audioUnit: AUAudioUnit) {
    ...
    let content = ATATExtensionMainView(parameterTree: observableParameterTree)
